...
def process_excel_file(file_path, user_id):
    ...
    try:
        ...
        for sheet_name, df in xf.items():
            ...
            # Buscar ROL (exactamente "ROL")
            for col in df.columns:
                if col.strip().upper() == 'ROL':
                    column_mapping['rol'] = col
                    print(f"🏷️ Columna de rol encontrada: {col}")
                    break

            # Buscar PREDIO/ORIGEN (exactamente "PREDIO/ORIGEN")
            for col in df.columns:
                if col.strip().upper() == 'PREDIO/ORIGEN':
                    column_mapping['origen'] = col
                    print(f"🌲 Columna de predio/origen encontrada: {col}")
                    break

            # Buscar COMUNA (exactamente "COMUNA")
            for col in df.columns:
                if col.strip().upper() == 'COMUNA':
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    # Obtener ROL de la columna "ROL" (opcional)
                    rol = None
                    if 'rol' in column_mapping and pd.notna(row[column_mapping['rol']]):
                        rol_raw = str(row[column_mapping['rol']]).strip()
                        # Eliminar comillas simples del rol y asignar al campo rol
                        rol = rol_raw.replace("'", "")
                        print(f"🏷️ Fila {index}: Rol procesado: '{rol_raw}' → '{rol}'")
                    else:
                        print(f"🏷️ Fila {index}: Sin rol especificado")

                    # Obtener PREDIO de la columna "PREDIO/ORIGEN" (opcional)
                    origen = None
                    if 'origen' in column_mapping and pd.notna(row[column_mapping['origen']]):
                        origen = str(row[column_mapping['origen']]).strip()
                        print(f"🌲 Fila {index}: Predio: {origen}")
                    else:
                        print(f"🌲 Fila {index}: Sin predio especificado")

                    # Obtener COMUNA de la columna "COMUNA" (opcional)
                    comuna = None
